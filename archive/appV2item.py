...
def build_voicing(root_pc, intervals, prev_notes=None):
    """
    Build a 4-voice SATB-style piano voicing.

    Voice ranges:
      Bass    : 36 – 52  (C2 – E3)  — always chord root
      Tenor   : 48 – 64  (C3 – E4)
      Alto    : 55 – 72  (G3 – C5)
      Soprano : 60 – 79  (C4 – G5)

    If prev_notes is given, every voice moves to its nearest available
    chord tone (strict voice-leading).
    """
    ...
    if not prev_notes:
        # Initial voicing: close position starting around C3-C4
        bass = nearest(bass_tones,  40)   # ~E2
        ten  = nearest(tenor_tones, 55)   # ~G3
        alt  = nearest(alto_tones,  60)   # C4
        sop  = nearest(sop_tones,   64)   # E4
    else:
    ...
    # Enforce ascending order (no voice crossing)
    voices = sorted([bass, ten, alt, sop])
    # Re-pin bass to bass register if sorting shifted it
...
def voice_lead_progression(chords_abs):
    ...
    Maintains voice-leading state across the progression.
    """
    result     = []
    prev_notes = None

    for chord_name in chords_abs:
        # Menangkap token Jeda
        if chord_name == "[GAP]":
...
def create_midi(chords_abs, tempo_label, filename="output.mid"):
    ...
    rest_ticks = 0 # Akumulator jeda ketukan
    for v in voiced:
        notes = v['notes']
        vels  = v['velocities']
        n     = len(notes)

        # Jika bertemu GAP, tambahkan durasi istirahat dan lewati note_on
        if not notes or v['chord'] == '[GAP]':
            rest_ticks += ticks_per_chord
            continue

        # --- note_on with arpeggiation roll ---
        for i, (note, vel) in enumerate(zip(notes, vels)):
            t = arp_ticks if i > 0 else 0
            if i == 0:
                t += rest_ticks # Terapkan jeda pada nada pertama chord ini
            track.append(Message('note_on', note=note, velocity=vel, time=t))

        rest_ticks = 0 # Reset akumulator setelah digunakan

        # --- note_off ---
        hold = ticks_per_chord - arp_ticks * (n - 1) - 20  
...
def build_chord_events(song_data, tempo_label):
    ...
    for entry in song_data:
        ...
        # Tambahkan Jeda (GAP) di akhir setiap bagan lagu
...
def generate_piano_player_html(song_data, tempo_label):
    """Return a self-contained HTML string with a Web Audio piano player."""
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .btn.active {{ background: #C8A45C22; border-color: #C8A45C; color: #C8A45C; }}
  ...
  .chord-pill:hover {{ border-color: #444; color: #EDE9E2; }}
  .chord-pill.playing {{
    background: #C8A45C22;
    border-color: #C8A45C;
    color: #C8A45C;
...
<div class="player" id="player">
  ...
  <div class="controls">
    ...
    <div class="now-playing">
      <div class="np-chord" id="npChord">Tekan ▶ untuk memutar</div>
...
(function() {{
  ...
  function getPills() {{
    return document.querySelectorAll('.chord-pill');
  }}
  ...
  // ── Piano synthesis ──────────────────────────
  // Additive synthesis: fundamental + harmonics with ADSR envelope
  function midiToFreq(midi) {{
    return 440 * Math.pow(2, (midi - 69) / 12);
  }}

  function playNote(ctx, freq, startTime, duration, velocity) {{
    // Additive synthesis — triangle fundamental + sine harmonics (piano-like)
    const partials = [
      [1, 1.00, 'triangle'],
      [2, 0.45, 'sine'],
      [3, 0.18, 'sine'],
    ...
    // Subtle random detune per note for natural feel
    ...
    partials.forEach(([h, amp, type]) => {{
      ...
      const peak    = (velocity / 127) * amp * 0.22;
  ...
  function playChord(ctx, notes, velocities, startTime, duration) {{
    const arpDelay = 0.018; // 18 ms roll between voices (bass → soprano)
    notes.forEach((midi, i) => {{
      const vel  = (velocities && velocities[i]) ? velocities[i] : 70;
      const freq = 440 * Math.pow(2, (midi - 69) / 12);
      const t    = startTime + i * arpDelay;
      // Bass holds slightly longer for warmth
  ...
  let startIndex = 0;   // which event index we started from
  let rafId      = null;

  function totalTimeUpTo(index) {{
    let t = 0;
    for (let i = 0; i < index; i++) t += events[i].duration;
    return t;
  }}
  ...
  function scheduleAll(ctx, fromIndex, startAudioTime) {{
    let offset = 0;
    for (let i = fromIndex; i < events.length; i++) {{
      playChord(ctx, events[i].notes, events[i].velocities, startAudioTime + offset, events[i].duration);
      offset += events[i].duration;
    }}
  }}
  ...
  function updateUI(elapsed) {{
    ...
    if (idx >= 0 && idx < events.length) {{
      document.getElementById('npChord').textContent   = events[idx].chord;
      document.getElementById('npSection').textContent = events[idx].section;
    }}

    // Update pills
    const pills = getPills();
    pills.forEach((p, i) => {{
      p.classList.toggle('playing', i === idx && playing);
    }});
  }}
  ...
  function startPlayback(fromIndex) {{
    ...
    scheduleAll(ctx, fromIndex, startedAt);
    playing    = true;
  ...
  function resumePlayback() {{
    const ctx = getCtx();
    const elapsed = totalTimeUpTo(startIndex); // time since last seek
    // Resume from correct position
    const currentElapsed = (ctx.state === 'suspended')
      ? (performance.now() / 1000 - startedAt) + totalTimeUpTo(startIndex) // rough
      : 0;

    // Simplest: restart from current chord index
    const progressBar = document.getElementById('progressBar');
    const pct = parseFloat(progressBar.style.width) / 100;
    const seekElapsed = pct * totalDuration;
    const idx = indexAtTime(seekElapsed);
  ...
  function stopPlayback() {{
    ...
    // Close and recreate context to cut all sound immediately
    if (audioCtx) {{
      audioCtx.close();
      audioCtx = null;
    }}
    ...
    document.getElementById('btnPlay').textContent = '▶';
    document.getElementById('btnPlay').classList.remove('active');
    document.getElementById('npChord').textContent   = 'Tekan ▶ untuk memutar';
    document.getElementById('npSection').textContent = '—';
    document.getElementById('progressBar').style.width = '0%';
    document.getElementById('timeDisplay').textContent =
      '0:00 / ' + formatTime(totalDuration);
    getPills().forEach(p => p.classList.remove('playing'));
  }}
  ...
  function seekTo(index) {{
    ...
    updateUI(totalTimeUpTo(index));
    if (wasPlaying) startPlayback(index);
    else {{
      document.getElementById('btnPlay').textContent = '▶';
      document.getElementById('btnPlay').classList.remove('active');
    }}
...
:root {
    ...
    --text-3:     #5C574F;
    --accent:     #C8A45C;
    --accent-dim: rgba(200,164,92,0.10);
    --r:          8px;
}
...
<div class="app-header">
    ...
    <h1 class="app-title">ChordCraft</h1>
    <p class="app-subtitle">Buat progresi chord untuk lagumu · Hybrid Transformer-LSTM</p>
</div>
...
st.markdown("""<div class="step-label">
    ...
    <span class="step-title">Suasana Lagu</span>
...
# ── STEP 2 — Nada Dasar & Kecepatan ─────────
st.markdown("""<div class="step-label">
    <span class="step-num">2</span>
    <span class="step-title">Nada Dasar &amp; Kecepatan</span>
</div>""", unsafe_allow_html=True)
...
st.markdown("""<div class="step-label">
    ...
    <span class="step-title">Struktur &amp; Kreativitas</span>
</div>""", unsafe_allow_html=True)
...
st.markdown(f"""<div class="creativity-labels">
    <span class="creativity-lbl {'active' if creativity_level == 1 else ''}">Konvensional</span>
    <span class="creativity-lbl {'active' if creativity_level == 2 else ''}">Seimbang</span>
    <span class="creativity-lbl {'active' if creativity_level == 3 else ''}">Eksperimental</span>
</div>""", unsafe_allow_html=True)
...
if generate_clicked:
    ...
    if not song_structure:
        st.warning("Pilih minimal satu seksi lagu terlebih dahulu.")
        st.stop()

    with st.spinner("Sedang membuat progresi chord..."):
        song_data       = []
        all_abs         = []
        seen_sections   = {}
        ...
        for section in song_structure:
            ...
            # Data untuk UI (Kartu Chord) - Tanpa GAP agar tetap bersih
            song_data.append({"section": section, "roman": roman, "abs": abs_c})
            
            # Data untuk MIDI - Sisipkan GAP di setiap akhir seksi
            all_abs.extend(abs_c)
            all_abs.append("[GAP]")

        # Pembuatan MIDI kini menerima daftar chord yang sudah ada GAP-nya
...
<div class="result-header">
    <div class="result-title">Hasil Generasi</div>
    <div class="result-meta">{total_sections} seksi &nbsp;·&nbsp; {total_chords} chord</div>
</div>
<div class="config-bar">
    <div class="config-item">
        <span class="config-key">Suasana</span>
        <span class="config-val">{cfg["mood"]}</span>
    </div>
    <div class="config-item">
        <span class="config-key">Nada Dasar</span>
    ...
    <div class="config-item">
        <span class="config-key">Struktur</span>
        <span class="config-val">{cfg["structure"]}</span>
    </div>
    <div class="config-item">
        <span class="config-key">Kreativitas</span>
    ...
    if st.session_state.midi_path and os.path.exists(st.session_state.midi_path):
        with open(st.session_state.midi_path, "rb") as f:
            st.download_button(
                label="⬇  Download MIDI",
                data=f,
                file_name="chordcraft_output.mid",
                mime="audio/midi",
